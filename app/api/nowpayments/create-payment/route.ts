...
export async function POST(request: NextRequest) {
  try {
    ...
    const nowPaymentsRequest = {
      ...
      ipn_callback_url: `${process.env.NEXT_PUBLIC_APP_BASE_URL || 'http://localhost:3000'}/api/webhooks/nowpayments`,
      success_url: `${process.env.NEXT_PUBLIC_APP_BASE_URL || 'http://localhost:3000'}/dashboard/deposit?payment=success`,
      cancel_url: `${process.env.NEXT_PUBLIC_APP_BASE_URL || 'http://localhost:3000'}/dashboard/deposit?payment=cancelled`,
    };

    console.log('Creating NowPayments invoice:', { orderId, price_amount, pay_currency });
